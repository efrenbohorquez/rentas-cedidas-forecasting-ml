...
**Resultados:**
El análisis comparativo identificó al algoritmo **XGBoost** como el modelo más eficiente, logrando reducir el Error Porcentual Absoluto Medio (**MAPE**) a niveles inferiores al **12%** en horizontes agregados. A diferencia de los modelos tradicionales, XGBoost demostró una capacidad superior para capturar los picos estacionales críticos de enero y julio, característicos del calendario tributario regional. Por el contrario, los modelos de redes neuronales (LSTM) mostraron limitaciones para generalizar patrones en series temporales cortas con alta variabilidad.
